...
export async function handleGLEvent(payload: EachMessagePayload): Promise<void> {
  ...
  await query(
    `INSERT INTO gl_period_balances
     ...
     VALUES ($1, $2, COALESCE($3, '00000000-0000-0000-0000-000000000000'::uuid),
             $4, $5, $6, $7, $8, $9, $8 - $9, 1)
     ON CONFLICT (entity_id, period_id, fund_id, node_ref_id, economic_category)
     DO UPDATE SET
       debit_total = gl_period_balances.debit_total + $8,
       credit_total = gl_period_balances.credit_total + $9,
       net_balance = (gl_period_balances.debit_total + $8) - (gl_period_balances.credit_total + $9),
