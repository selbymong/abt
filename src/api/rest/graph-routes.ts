...
graphRouter.post('/edges/contributes-to', async (req: Request, res: Response) => {
  try { await createContributesToEdge(req.body); res.status(201).json({ success: true }); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});

graphRouter.get('/edges/contributes-to/from/:sourceId', async (req: Request, res: Response) => {
  try { res.json({ edges: await getContributesToEdges(req.params.sourceId as string) }); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});

graphRouter.get('/edges/contributes-to/to/:targetId', async (req: Request, res: Response) => {
  try { res.json({ edges: await getIncomingContributesToEdges(req.params.targetId as string) }); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});
...
graphRouter.post('/edges/depends-on', async (req: Request, res: Response) => {
  try { await createDependsOnEdge(req.body); res.status(201).json({ success: true }); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});

graphRouter.get('/edges/depends-on/from/:sourceId', async (req: Request, res: Response) => {
  try { res.json({ edges: await getDependsOnEdges(req.params.sourceId as string) }); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});
...
graphRouter.post('/edges/delegates-to', async (req: Request, res: Response) => {
  try { await createDelegatesToEdge(req.body); res.status(201).json({ success: true }); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});

graphRouter.get('/edges/delegates-to/from/:sourceId', async (req: Request, res: Response) => {
  try { res.json({ edges: await getDelegatesToEdges(req.params.sourceId as string) }); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});
...
graphRouter.post('/edges/prohibits', async (req: Request, res: Response) => {
  try { await createProhibitsEdge(req.body); res.status(201).json({ success: true }); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});

graphRouter.get('/edges/prohibits/from/:constraintId', async (req: Request, res: Response) => {
  try { res.json({ edges: await getProhibitsEdges(req.params.constraintId as string) }); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});
...
graphRouter.get('/prohibitions/:activityId', async (req: Request, res: Response) => {
  try { res.json(await checkProhibitions(req.params.activityId as string)); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});
...
graphRouter.post('/prohibitions/filter', async (req: Request, res: Response) => {
  try {
    ...
    res.json(await filterProhibitedActivities(activityIds));
  } catch (err: any) { res.status(500).json({ error: err.message }); }
});
...
graphRouter.get('/obligations/alerts/:entityId', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ alerts: await getObligationAlerts(req.params.entityId as string, horizonDays) });
  } catch (err: any) { res.status(500).json({ error: err.message }); }
});
...
graphRouter.get('/obligations/by-entity/:entityId', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ obligations: await getEntityObligations(req.params.entityId as string, statusFilter) });
  } catch (err: any) { res.status(500).json({ error: err.message }); }
});
